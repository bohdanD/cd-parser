...
export class FilterComponent implements OnInit {
  ...
  token: string = '4209690b06efe647d15006e0f3b204e32bfee8b804cecf2cc23679ef4d6c13b0';
        ...
        (res: any) => {
          if (Number(res.count)) {
            ...
            if (carId !== this.latestCar.carId) {
              ...
              if (this.email) {
                this.listingService.sendMail(
                  {
                    SendTo: this.email,
                    CarModel: carModel,
                    Price: Number(latest.price),
                    Pickup: this.pickCity,
                    Destination: this.destCity,
                    Phone: latest.phone,
                    PricePerMile: latest.pricePerMile,
                  }
                ).subscribe(() => {});
              }
            }
